...
#define portable_static_assert(cond, msg) _Static_assert(cond, msg)
#else
#define portable_static_assert(cond, msg) static_assert(cond, msg)
#endif

// char(*__kaboom)[sizeof(func_prop_t)] = 1;

// Compile-time assert the property structs are 64 bits.
portable_static_assert(sizeof(func_prop_t) == 8,
                       "Size of func_prop_t is not 64 bits.");
portable_static_assert(sizeof(func_exit_prop_t) == 8,
                       "Size of func_exit_prop_t is not 64 bits.");
portable_static_assert(sizeof(bb_prop_t) == 8,
                       "Size of bb_prop_t is not 64 bits.");
portable_static_assert(sizeof(call_prop_t) == 8,
                       "Size of call_prop_t is not 64 bits.");
portable_static_assert(sizeof(load_prop_t) == 8,
                       "Size of load_prop_t is not 64 bits.");
portable_static_assert(sizeof(store_prop_t) == 8,
...
static void ensure_fed_table_capacity(fed_type_t fed_type,
  ...
  if (total_num_entries > 0) {
    table->entries = (source_loc_t *)realloc(
        table->entries, total_num_entries * sizeof(source_loc_t));
...
static void ensure_sizeinfo_table_capacity(sizeinfo_type_t sizeinfo_type,
  ...
  int64_t total_num_entries = table->num_entries + num_new_entries;
  if (total_num_entries > 0) {
    table->entries = (sizeinfo_t *)realloc(
        table->entries, total_num_entries * sizeof(sizeinfo_t));
    table->num_entries = total_num_entries;
    assert(table->entries != NULL);
  }
}
...
static inline instrumentation_counts_t
...
// before main().
CSIRT_API void
__csirt_unit_init(const char *const name, unit_fed_table_t *unit_fed_tables,
  ...
  for (unsigned i = 0; i < NUM_FED_TYPES; i++) {
    add_fed_table(i, unit_fed_tables[i].num_entries,
